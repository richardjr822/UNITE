    ...
    @if ($isAdmin)
        ...
        <section class="student-canvas">
            ...
            <div class="admin-empty-board">
                <div class="overflow-hidden rounded-2xl border border-[#d4eadf] bg-white">
                    <table class="min-w-full divide-y divide-[#e8f4ee]">
                        <thead>
                            <tr class="bg-[#f3faf6]">
                                <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#385e52]">Title</th>
                                <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#385e52]">Date</th>
                                <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#385e52]">Venue</th>
                                <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-[#385e52]">Registered</th>
                                <th class="px-6 py-3 text-right text-xs font-bold uppercase tracking-wider text-[#385e52]">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-[#e8f4ee]">
                            @forelse ($events as $event)
                                <tr class="hover:bg-[#f8fdfb] transition-colors">
                                    <td class="px-6 py-4 text-sm font-semibold text-[#1b3a2f]">
                                        <button type="button"
                                            data-view-event
                                            ...
                                            class="text-left font-semibold text-[#1b3a2f] hover:text-[#2a7a5a]">{{ $event->title }}</button>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-[#385e52]">
                                        {{ $event->date->format('D, M d, Y') }} · {{ \Illuminate\Support\Carbon::parse($event->time)->format('H:i') }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-[#385e52]">{{ $event->venue }}</td>
                                    <td class="px-6 py-4 text-sm text-[#385e52]">
                                        <span class="flex items-center gap-1.5">
                                            <svg class="h-4 w-4 text-[#2a7a5a]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <button type="button"
                                                ...
                                                data-capacity="{{ $event->capacity }}"
                                                data-status="{{ $event->status }}"
                                                class="inline-flex items-center justify-center rounded-lg p-1.5 text-[#385e52] hover:bg-[#dff2ea] transition-colors">
                                            ...
                                            <form method="POST" action="{{ route('events.destroy', $event) }}" data-confirm-submit data-confirm-title="Delete Event" data-confirm-message="Delete {{ $event->title }}? This action cannot be undone.">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center justify-center rounded-lg p-1.5 text-[#8d2d2d] hover:bg-[#f9dfdf] transition-colors">
                                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-sm text-[#385e52]">No events matched your filters.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <div class="mt-4 flex items-center gap-3">
                            ...
                            @else
                                <button type="button"
                                    ...
                                    class="student-mini-btn">Register</button>
                            @endif
                            ...
                            <button type="button"
                                ...
                                data-status="{{ $event->status }}"
                                data-can-register="0"
                                class="student-mini-btn">View Details</button>
                        </div>
                    </article>
                @empty
