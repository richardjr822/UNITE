    ...
    @if ($isAdmin)
        ...
        <section class="student-canvas">
            ...
            <div class="overflow-hidden rounded-2xl border border-[#d6e5df] bg-white shadow">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <a href="{{ route('events.show', $event) }}"
                                                class="inline-flex items-center justify-center rounded-lg p-2 text-[#2f9b74] hover:bg-[#e8f4ee] transition-colors" title="View participants">
                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @if ($event->date->isToday() && $event->status === 'scheduled')
                                                <form method="POST" action="{{ route('events.markDone', $event) }}" data-confirm-submit data-confirm-title="Mark as Done" data-confirm-message="Mark '{{ $event->title }}' as done? This cannot be undone." class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="inline-flex items-center justify-center rounded-lg p-2 text-blue-600 hover:bg-blue-50 transition-colors" title="Mark as Done">
                                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @if ($events->hasPages())
                    <div class="px-6 py-4 border-t border-[#e8f4ee] flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @else
                                <a href="{{ $events->previousPageUrl() }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[#d6e5df] text-[#2f9b74] hover:bg-[#e8f5ee] transition">
                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                                </a>
                            @endif
                            ...
                            @if ($events->hasMorePages())
                                <a href="{{ $events->nextPageUrl() }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-[#d6e5df] text-[#2f9b74] hover:bg-[#e8f5ee] transition">
            ...
            @if ($events->hasPages())
                <div class="mt-5 flex items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        @else
                            <a href="{{ $events->previousPageUrl() }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white/10 border border-white/30 text-white hover:bg-white/20 transition">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                            </a>
                        @endif
                        @foreach ($events->getUrlRange(max(1, $events->currentPage() - 2), min($events->lastPage(), $events->currentPage() + 2)) as $page => $url)
                        ...
                        @endforeach
                        @if ($events->hasMorePages())
                            <a href="{{ $events->nextPageUrl() }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-white/10 border border-white/30 text-white hover:bg-white/20 transition">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                            </a>
                        @else
                            <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-white/30 cursor-not-allowed">
